...
import Animated, { useSharedValue, useAnimatedStyle, withTiming, withRepeat, withSequence, withDelay, withSpring, runOnJS, Easing, interpolate, Extrapolation, } from "react-native-reanimated";
// ---------------------------------------------------------------------------
 ...
 * ```
 */
export function useReanimatedText(config) {
    const { animation, duration = 600, delay = 0, loop = false, onStart, onComplete, } = config;
    const progress = useSharedValue(0);
    const slideX = useSharedValue(0);
    const slideY = useSharedValue(0);
    const scale = useSharedValue(1);
    useEffect(() => {
        const timing = (toValue) => withTiming(toValue, {
            duration,
        ...
        const runAnim = () => {
            switch (animation) {
                ...
                case "pulse":
                    if (loop) {
                        scale.value = withRepeat(withSequence(withTiming(1.1, { duration: duration / 2 }), withTiming(1.0, { duration: duration / 2 })), -1, true);
                    }
                    else {
                        scale.value = withSequence(withTiming(1.1, { duration: duration / 2 }), withTiming(1.0, { duration: duration / 2 }, doneFn));
                    }
                    progress.value = 1;
                    break;
                case "shake":
                    slideX.value = withRepeat(withSequence(withTiming(-8, { duration: duration / 6 }), withTiming(8, { duration: duration / 6 }), withTiming(-4, { duration: duration / 6 }), withTiming(4, { duration: duration / 6 }), withTiming(0, { duration: duration / 6 })), loop ? -1 : 1, false, doneFn);
                    progress.value = 1;
                    break;
                case "bounce":
                    slideY.value = withRepeat(withSequence(withTiming(-16, { duration: duration / 2 }), withTiming(0, { duration: duration / 2 })), loop ? -1 : 1, false, doneFn);
                    progress.value = 1;
                    break;
            }
        };
 ...
 * ```
 */
export const ReanimatedAppText = memo(({ animation, duration, delay, loop, onStart, onComplete, children, style, testID }) => {
    const animatedStyle = useReanimatedText({
        animation,
        duration,
        delay,
    ...
    return (<Animated.Text style={[style, animatedStyle]} testID={testID}>
        {children}
      </Animated.Text>);
});
 ...
 */
export function isReanimatedAvailable() {
