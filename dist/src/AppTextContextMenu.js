...
function computeMenuPosition(touchX, touchY, actionCount) {
    const menuH = actionCount * ITEM_HEIGHT + MENU_PADDING * 2 + actionCount; // separators
    let top = touchY - menuH - 12;
    if (top < 20)
        top = touchY + 24; // flip below if no room above
    let left = touchX - MENU_WIDTH / 2;
    if (left < 8)
        left = 8;
    if (left + MENU_WIDTH > SCREEN_W - 8)
        left = SCREEN_W - MENU_WIDTH - 8;
    return { top, left };
}
// ---------------------------------------------------------------------------
// Component
// ---------------------------------------------------------------------------
export const AppTextContextMenu = memo(({ actions, children, longPressDelay = 500, onMenuOpen, onMenuClose, backdropOpacity = 0.3, menuBackgroundColor = "#FFFFFF", actionTextColor = "#1F2937", destructiveColor = "#EF4444", }) => {
    const [visible, setVisible] = useState(false);
    const [position, setPosition] = useState({ top: 100, left: 80 });
    const longPressXY = useRef({ x: 0, y: 0 });
    ...
    return (<>
        <Pressable onLongPress={handleLongPress} delayLongPress={longPressDelay} android_ripple={null}>
          {children}
        </Pressable>

        <Modal visible={visible} transparent statusBarTranslucent animationType="fade" onRequestClose={handleClose} accessible={false}>
          {/* Backdrop — tap to dismiss */}
          <Pressable style={[styles.backdrop, { opacity: backdropOpacity }]} onPress={handleClose}/>

          {/* Menu card */}
          <View style={menuStyle} accessible accessibilityViewIsModal>
            {actions.map((action, index) => (<React.Fragment key={`menu-action-${index}`}>
                {index > 0 && <View style={styles.separator}/>}
                <TouchableOpacity style={[styles.actionRow, action.disabled && styles.actionDisabled]} onPress={action.disabled ? undefined : () => handleActionPress(action)} activeOpacity={0.6} accessibilityRole="button" accessibilityLabel={action.label} accessibilityState={{ disabled: action.disabled }}>
