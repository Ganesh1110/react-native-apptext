...
export interface AppTextDevToolsProps {
...
interface StatsSnapshot {
  ...
  cacheHits: number;
  cacheMisses: number;
  topKeys: Array<{ name: string; p95: number; count: number }>;
  language: string;
  direction: string;
}
...
function getPositionStyle(position: DevToolsPosition) {
  const base: Record<string, number | string> = {
    position: "absolute",
    zIndex: 9999,
    maxWidth: 320,
  };
  if (position === "top-left") return { ...base, top: 48, left: 8 };
  if (position === "top-right") return { ...base, top: 48, right: 8 };
  if (position === "bottom-left") return { ...base, bottom: 64, left: 8 };
  return { ...base, bottom: 64, right: 8 };
}

function hitRateColor(rate: number): string {
  if (rate >= 0.9) return "#4ade80"; // green
  if (rate >= 0.6) return "#fbbf24"; // amber
  return "#f87171"; // red
}
...
export const AppTextDevTools = memo<AppTextDevToolsProps>(
  ({
...
const DevToolsInner = memo<Required<AppTextDevToolsProps>>(
  ({ position, refreshInterval, defaultCollapsed }) => {
    ...
    const posStyle = getPositionStyle(position) as any;
    ...
    return (
      <View style={[styles.container, posStyle]} pointerEvents="box-none">
        ...
        <Pressable
          ...
          accessibilityLabel="Toggle AppText DevTools"
        >
...
const Row = memo<{ label: string; value: string; valueColor?: string }>(
