...
export type AnimationValues = {
  ...
  glow: Animated.Value;
  neon: Animated.Value;
  gradient: Animated.Value;
};

export type AnimationGenerator = (
  values: AnimationValues,
  config: { duration: number; delay: number; effectiveDuration: number },
) => Animated.CompositeAnimation;

export const ANIMATION_REGISTRY: Record<string, AnimationGenerator> = {
  // === ENTRANCE ANIMATIONS ===
  fade: (v, { duration, delay }) =>
  ...
  rubberBand: (v, { effectiveDuration }) =>
    Animated.loop(
      Animated.sequence([
        Animated.timing(v.scale, {
          toValue: 1.25,
