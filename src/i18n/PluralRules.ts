...
export const PLURAL_RULES: Record<string, (count: number) => PluralForm> = {
  // Germanic
  en: (n) => (n === 1 ? "one" : "other"),
  de: (n) => (n === 1 ? "one" : "other"),
  nl: (n) => (n === 1 ? "one" : "other"),
...
export const ORDINAL_RULES: Record<string, (count: number) => PluralForm> = {
  en: (n) => {
    const mod10 = n % 10;
    const mod100 = n % 100;
    if (mod10 === 1 && mod100 !== 11) return "one";
...
export function getPluralForm(language: string, count: number): PluralForm {
  ...
  const rule = PLURAL_RULES[langCode];
  if (!rule) return "other";
  return rule(Math.abs(Math.floor(count)));
}

export function getOrdinalForm(language: string, count: number): PluralForm {
  const langCode = language?.split("-")[0] || "en";
